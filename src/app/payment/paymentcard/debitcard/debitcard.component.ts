...
export class DebitcardComponent {
  ...
  constructor(private datePipe: DatePipe,private route:ActivatedRoute,private registerService:RegisterService,private router:Router,private dataservice:DataserviceService,private formBuilder: FormBuilder){
    this.seatid = this.dataservice.getSelectedSeats();
    this.paymentForm.patchValue({
      selectedseats: this.seatid
    });
    const today = new Date();
    this.currentDate = this.datePipe.transform(today, 'yyyy-MM');
  }
